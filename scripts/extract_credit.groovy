...
def oWriter = new BufferedWriter(new FileWriter('data/credit_annotations.tsv'))
oWriter.write("oid\tclass\tproperty\tvalue\torcid\n")
    ...
    EntitySearcher.getAnnotationAssertionAxioms(cl, ont).each { annoAxiom ->
      ...
      if(PROPS.contains(property)) {
        ...
        if(val instanceof OWLLiteral) {
          ...
          oWriter.write([id, iri, property, literal, orcid].join('\t') + '\n')
        }
      }
    }
  }
...
def out = allClasses.collect { k, v -> "$k\t${v.join(',')}" }.join('\n')
new File('data/all_classes.txt').text = out
